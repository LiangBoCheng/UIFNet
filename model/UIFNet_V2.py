...
class DynamicScaleSelection(nn.Module):
    def __init__(self, channel):
        ...
        self.selector = nn.Sequential(
            ...
            nn.Conv2d(channel // 4, 4, 1, bias=False)
        )

        # residual
        self.conv_cat = BasicConv2d(channel, channel, 3, padding=1)
...
class Decoder(nn.Module):
    def __init__(self, channel):
        ...
        self.dss1 = DynamicScaleSelection(channel)
        self.dss2 = DynamicScaleSelection(channel)
        self.dss3 = DynamicScaleSelection(channel)
        self.dss4 = DynamicScaleSelection(channel)
        self.dss5 = DynamicScaleSelection(channel)
    ...
    def forward(self, x_sal1, x_sal2, x_sal3, x_sal4, x_sal5):
        ...
        x_sal1 = self.dss1(x_sal1)
        x_sal2 = self.dss2(x_sal2)
        x_sal3 = self.dss3(x_sal3)
        x_sal4 = self.dss4(x_sal4)
        x_sal5 = self.dss5(x_sal5)
...
class UIFNet(nn.Module):
    def __init__(self, pretrained=True, channel=32):

        super(UIFNet, self).__init__()
    ...
    def forward(self, x):
        ...
        for cycle in range(2):
            ...
            uncertainty = (1 - torch.abs(mask - 0.5) * 2).detach()
            x_sal1, x_sal2, x_sal3, x_sal4, x_sal5 = self.backbone(x, uncertainty)
            x_out = self.decoder(x_sal1, x_sal2, x_sal3, x_sal4, x_sal5)
            sal_out = F.interpolate(x_out, size=size, mode='bilinear', align_corners=True)
            x_res.append(sal_out)
            x_sigmoid.append(self.sigmoid(sal_out))
