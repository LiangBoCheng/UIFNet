...
from model.UIFNet_V3 import UIFNet
# from model.UIFNet_V2 import UIFNet
...
image_root = '/root/user/cs_tcci_liangbocheng/SOD_Datasets/' + opt.trainset + '/train-images/'
gt_root = '/root/user/cs_tcci_liangbocheng/SOD_Datasets/' + opt.trainset + '/train-labels/'
...
def train(train_loader, model, optimizer, epoch):
    ...
    save_path = 'models/UIFNet/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    if (epoch+1) > 40:
        torch.save(model.state_dict(), save_path + 'UIFNet_' + opt.trainset + '.pth' + '.%d' % epoch)
